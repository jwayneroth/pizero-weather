...
"""
def text_with_border(string, xy, align="left"):
	over = 2
	x,y = xy
	if align is "right":
...
if __name__ == '__main__':
	...
	text_with_border(date, (LR_PADDING, TB_PADDING - FONT_Y_OFFSET))
	text_with_border(time, (pzwglobals.DISPLAY_WIDTH - LR_PADDING - 2, TB_PADDING - FONT_Y_OFFSET), align="right")
	...
	if current is not None:
		if "temperature" in current:
			...
			text_with_border(temp_str, (pzwglobals.DISPLAY_WIDTH - LR_PADDING - 12, mid_y), align="right")

		if "humidity" in current:
			bottom_y = pzwglobals.DISPLAY_HEIGHT - TB_PADDING -  draw.textsize(temp_str, font=font)[1]
			text_with_border("{} %".format(current["humidity"]), (pzwglobals.DISPLAY_WIDTH - LR_PADDING, bottom_y), align="right")
	...
	while (True):
		...
		if (input_queue.qsize() > 0):
			...
			logger.info("input_str = {}".format(input_str))
			
			# "x" input triggers exit, any other input triggers screen change
			if (input_str == EXIT_COMMAND):
				logger.info("Exiting serial terminal.")
				kill()
			else:
